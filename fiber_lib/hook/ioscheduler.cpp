...
namespace sylar{
    ...
    IOManager::FdContext::EventContext& IOManager::FdContext::getEventContext(Event event){
        ...
        throw std::invalid_argument("Unsuppoted event type");
    }
    ...
    void IOManager::FdContext::triggerEvent(IOManager::Event event){
        ...
        if(ctx.cb){
            // call SchedulerTask(std::function<void()>* f, int thr)
            ctx.scheduler->scheduleLock(&ctx.cb);
        } else {
            // call SchedulerTask(std::function<Fiber>* f, int thr)
    ...
    bool IOManager::cancelEvent(int fd, Event event){
        ...
        // update fdcontext, event context and trigger, 和dlelete区别在这里
