...
namespace sylar{
    ...
    std::shared_ptr<Fiber> Fiber::GetThis(){
        ...
        t_scheduler_fiber = main_fiber.get(); // 除非主动设置 主协程默认调度协程
    ...
    void Fiber::resume(){
        ...
        if(m_runInScheduler) // 调度器 or 协程，根据是否使用调度器设置为准
            {
                SetThis(this);
        ...
        if(m_runInScheduler) // 调度器 or 协程，根据是否使用调度器设置为准
            {
                ...
                {
                    std::cerr << "yiled() to t_scheduler_fiber failed\n";
                    pthread_exit(NULL);
                }		
            }
            else
            {
                ...
                {
                    std::cerr << "yiled() to t_thread_fiber failed\n";
                    pthread_exit(NULL);
                }	
            }
    }
